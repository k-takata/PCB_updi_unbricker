...
|L1         |   1|330μH|100μH - 1.5mH, ≧100mA, [AL0307-331K](https://akizukidenshi.com/catalog/g/g103968/)等|
...
|R5,R15     |   2|4.7kΩ|黄紫赤金、1/4Wサイズ|
|R6         |   1|5.1Ω |緑茶金金、1/4Wサイズ|
...
UPDIピンからは12Vのパルスが出力されますので、接続先はそのことを考慮した回路になっている必要があります。

J6はAVR Dx/Exシリーズ用のコネクターで、[AVR Programming Adapter](https://www.microchip.com/en-us/development-tool/AC31S18A)と同じUPDI v2コネクターとなっています。
...
J3は、一般的な6pinのTTLシリアルコネクターとなっています。J3の6番ピンの機能はJ4にジャンパーピンを挿すことでRTSかDTRのどちらかを選択できます。
...
SW1は、HVプログラミングを使用するかどうかを選択します。HVプログラミングを使用しない場合は安全のためにOFFにしておくことをお勧めします。

SW3は、本機から電源を供給するかどうかを選択します。シルクのONの側に倒すと電源を供給し、OFFの側に倒すと供給しません。ターゲットに対して別の経路で電源を供給済みの場合はOFFにしてください。  
供給する電圧はSW2で選択できます。5Vまたは3.3Vを選択できます。  
供給する電圧によってLEDの色が変わります。3.3Vで緑色、5Vでオレンジ色（緑 + 赤）となります。SW3がOFFでターゲットを接続していない場合はLEDは消灯しますが、ターゲットを接続した場合はターゲットからの電源でLEDが点灯します。
...
MC34063Aを使用して12Vを発生させています。このICは1.5Aまで出力できますが、今回はそれほどの電流は不要なので、10mA程度を出力できるような構成にしています。

R10, R11 と R12 による分圧回路の抵抗の比は、8.6に近い値になるように調整します。手持ちの1%抵抗で8.6に近い値になる組み合わせが、56kΩ, 39kΩ, 2.7kΩの組み合わせだったのでそのようにしていますが、一般的には13kΩと1.5kΩを使う場合が多いようです。

出力電圧は以下のように計算できます。

$$
V_{out} = 1.25 \times (1 + R_{ratio})
$$
